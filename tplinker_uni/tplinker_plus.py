...
class HandshakingTaggingScheme(object):
    def __init__(self, rel2id, max_seq_len, entity_type2id):
        ...
        self.rel_tags = {self.separator.join([rel, lt]) for rel in self.rel2id.keys() for lt in self.link_types}
        self.rel_tags = sorted(self.rel_tags)

        self.rel_tag2id = {t:idx for idx, t in enumerate(self.rel_tags)}
        self.id2rel_tag = {idx:t for t, idx in self.rel_tag2id.items()}
        
        self.ent_tag2id = entity_type2id
        self.id2ent_tag = {ind:ent for ent, ind in self.ent_tag2id.items()}
    ...
    def get_spots(self, sample):
        '''
        rel_matrix_spots: [(tok_pos1, tok_pos2, rel_tag_id), ]
        ent_matrix_spots: [(tok_pos1, tok_pos2, ent_tag_id), ]
        '''
        rel_matrix_spots = []
        ent_matrix_spots = []
        
        rel_spot_memory_set = set()
        def add_rel_spot(spot):
            memory = "{},{},{}".format(*spot)
            if memory not in rel_spot_memory_set:
                rel_matrix_spots.append(spot)
                rel_spot_memory_set.add(memory)
                
        ent_spot_memory_set = set()
        def add_ent_spot(spot):
            memory = "{},{},{}".format(*spot)
            if memory not in ent_spot_memory_set:
                ent_matrix_spots.append(spot)
                ent_spot_memory_set.add(memory)

        for ent in sample["entity_list"]:
            add_ent_spot((ent["tok_span"][0], ent["tok_span"][1] - 1, self.ent_tag2id[ent["type"]]))
        ...
        for rel in sample["relation_list"]:
            ...
            if subj_tok_span[0] <= obj_tok_span[0]:
                add_rel_spot((subj_tok_span[0], obj_tok_span[0], self.rel_tag2id[self.separator.join([rel, "SH2OH"])]))
            else:
                add_rel_spot((obj_tok_span[0], subj_tok_span[0], self.rel_tag2id[self.separator.join([rel, "OH2SH"])]))
            if subj_tok_span[1] <= obj_tok_span[1]:
                add_rel_spot((subj_tok_span[1] - 1, obj_tok_span[1] - 1, self.rel_tag2id[self.separator.join([rel, "ST2OT"])]))
            else:
                add_rel_spot((obj_tok_span[1] - 1, subj_tok_span[1] - 1, self.rel_tag2id[self.separator.join([rel, "OT2ST"])]))
        return ent_matrix_spots, rel_matrix_spots

    def spots2shaking_tag(self, spots, tag_size):
        '''
        ...
        shaking_tag = torch.zeros(shaking_seq_len, tag_size).long()
        for sp in spots:
            shaking_idx = self.matrix_idx2shaking_idx[sp[0]][sp[1]]
            shaking_tag[shaking_idx][sp[2]] = 1
        return shaking_tag

    def spots2shaking_tag4batch(self, batch_spots, tag_size):
        '''
        ...
        '''
        shaking_seq_len = self.matrix_size * (self.matrix_size + 1) // 2
        batch_shaking_tag = torch.zeros(len(batch_spots), shaking_seq_len, tag_size).long()
    ...
    def decode_rel(self,
        ...
        rel_matrix_spots = self.get_spots_fr_shaking_tag(rel_shaking_tag)
...
#                 continue
        ...
        for sp in rel_matrix_spots:
            tag = self.id2rel_tag[sp[2]]
            rel_type, link_type = tag.split(self.separator) 
            if link_type == "ST2OT":
                tail_link_memory = self.separator.join([rel_type, str(sp[0]), str(sp[1])])
                tail_link_memory_set.add(tail_link_memory)
            elif link_type == "OT2ST":
                tail_link_memory = self.separator.join([rel_type, str(sp[1]), str(sp[0])])
                tail_link_memory_set.add(tail_link_memory)
        ...
        for sp in rel_matrix_spots:
            ...
            for subj in subj_list:
                for obj in obj_list:
                    tail_link_memory = self.separator.join([rel_type, str(subj["tok_span"][1] - 1), str(obj["tok_span"][1] - 1)])
...
class DataMaker4Bert():
    ...
    def get_indexed_data(self, data, max_seq_len, data_type = "train"):
        ...
        for ind, sample in tqdm(enumerate(data), desc = "Generate indexed train or valid data"):
            ...
            # tagging
            ent_spot_memory_set, rel_matrix_spots = None, None
            if data_type != "test":
                ent_matrix_spots, rel_matrix_spots = self.shaking_tagger.get_spots(sample)
    ...
    def generate_batch(self, batch_data, data_type = "train"):
        ...
        token_type_ids_list = [] 
        tok2char_span_list = []
        ent_matrix_spots_list = []
        rel_matrix_spots_list = []
        ...
        for tp in batch_data:
            ...
            if data_type != "test":
                ent_matrix_spots_list.append(tp[5])
                rel_matrix_spots_list.append(tp[6])

        batch_input_ids = torch.stack(input_ids_list, dim = 0)
        batch_attention_mask = torch.stack(attention_mask_list, dim = 0)
        batch_token_type_ids = torch.stack(token_type_ids_list, dim = 0)
        
        batch_ent_shaking_tag, batch_rel_shaking_tag = None, None
        if data_type != "test":
            batch_ent_shaking_tag = self.shaking_tagger.spots2shaking_tag4batch(ent_matrix_spots_list, self.shaking_tagger.get_ent_tag_size())
            batch_rel_shaking_tag = self.shaking_tagger.spots2shaking_tag4batch(rel_matrix_spots_list, self.shaking_tagger.get_rel_tag_size())

        return sample_list, \
              batch_input_ids, batch_attention_mask, batch_token_type_ids, tok2char_span_list, \
                batch_ent_shaking_tag, batch_rel_shaking_tag
...
class DataMaker4BiLSTM():
    ...
    def get_indexed_data(self, data, max_seq_len, data_type = "train"):
        ...
        for ind, sample in tqdm(enumerate(data), desc = "Generate indexed train or valid data"):
            text = sample["text"]
            # tagging
            ent_matrix_spots, rel_matrix_spots = None, None
            if data_type != "test":
                ent_matrix_spots, rel_matrix_spots = self.shaking_tagger.get_spots(sample)
    ...
    def generate_batch(self, batch_data, data_type = "train"):
        ...
        for tp in batch_data:
            ...
            if data_type != "test":
                ent_matrix_spots_list.append(tp[3])
                rel_matrix_spots_list.append(tp[4])

        batch_input_ids = torch.stack(input_ids_list, dim = 0)
        
        ent_batch_shaking_tag, rel_batch_shaking_tag = None, None
        if data_type != "test":
            ent_batch_shaking_tag = self.shaking_tagger.spots2shaking_tag4batch(ent_matrix_spots_list, self.shaking_tagger.get_ent_tag_size())
            rel_batch_shaking_tag = self.shaking_tagger.spots2shaking_tag4batch(rel_matrix_spots_list, self.shaking_tagger.get_rel_tag_size())
        
        return sample_list, \
                batch_input_ids, tok2char_span_list, \
                ent_batch_shaking_tag, rel_batch_shaking_tag
...
class TPLinkerPlusBert(nn.Module):
    def __init__(self, encoder, 
        ...
        self.ent_fc = nn.Linear(shaking_hidden_size, ent_tag_size)
        self.rel_fc = nn.Linear(shaking_hidden_size, rel_tag_size)
            
        # handshaking kernel
        self.handshaking_kernel_ent = HandshakingKernel(shaking_hidden_size, ent_shaking_type)
        self.handshaking_kernel_rel = HandshakingKernel(shaking_hidden_size, rel_shaking_type)
    ...
    def forward(self, input_ids, 
        ...
        ent_shaking_hiddens = self.handshaking_kernel_ent(last_hidden_state)
        rel_shaking_hiddens = self.handshaking_kernel_rel(last_hidden_state)
        
        def sample_tok_pairs2train(shaking_hiddens, sample_rate):
            # randomly sample segments of token pairs
            shaking_seq_len = shaking_hiddens.size()[1]
            segment_len = int(shaking_seq_len * sample_rate)
...
#             sampled_tok_pair_indices = torch.randint(shaking_seq_len, (shaking_hiddens.size()[0], segment_len))
            ...
            return shaking_hiddens, sampled_tok_pair_indices
        
        ent_sampled_indices, rel_sampled_indices = None, None
        if self.training: 
            ent_shaking_hiddens, ent_sampled_indices = sample_tok_pairs2train(ent_shaking_hiddens, self.tok_pair_sample_rate)
            rel_shaking_hiddens, rel_sampled_indices = sample_tok_pairs2train(rel_shaking_hiddens, self.tok_pair_sample_rate)
 
        # outputs: (batch_size, segment_len, tag_size) or (batch_size, shaking_seq_len, tag_size)
        ent_outputs = self.ent_fc(ent_shaking_hiddens)
        rel_outputs = self.rel_fc(rel_shaking_hiddens)

        return ent_outputs, ent_sampled_indices, rel_outputs, rel_sampled_indices
...
class TPLinkerPlusBiLSTM(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        # shaking_hiddens: (batch_size, shaking_seq_len, hidden_size)
        ent_shaking_hiddens = self.handshaking_kernel_ent(lstm_outputs)
        rel_shaking_hiddens = self.handshaking_kernel_rel(lstm_outputs)
...
#         return pred_rel_set, gold_rel_set, pred_ent_set, gold_ent_set
    ...
    def get_cpg(self, sample_list, 
                   tok2char_span_list, 
                   batch_pred_ent_shaking_tag, 
                   batch_pred_rel_shaking_tag, 
