...
ent_tag_size = handshaking_tagger.get_ent_tag_size()
rel_tag_size = handshaking_tagger.get_rel_tag_size()
...
if config["encoder"] == "BERT":
    ...
    rel_extractor = TPLinkerPlusBert(encoder, 
                                     ent_tag_size,
                                     rel_tag_size,
                                     hyper_parameters["ent_shaking_type"],
                                     hyper_parameters["rel_shaking_type"],
...
def train_step(batch_train_data, optimizer):
    if config["encoder"] == "BERT":
        sample_list, batch_input_ids,         batch_attention_mask, batch_token_type_ids,         tok2char_span_list, ent_batch_shaking_tag, rel_batch_shaking_tag = batch_train_data
        
        batch_input_ids,         batch_attention_mask,         batch_token_type_ids,         ent_batch_shaking_tag,         rel_batch_shaking_tag = (batch_input_ids.to(device), 
                              batch_attention_mask.to(device), 
                              batch_token_type_ids.to(device), 
                              ent_batch_shaking_tag.to(device),
                              rel_batch_shaking_tag.to(device)
                             )
        
    elif config["encoder"] in {"BiLSTM", }:
        sample_list, batch_input_ids,         tok2char_span_list, ent_batch_shaking_tag, rel_batch_shaking_tag = batch_train_data
        
        batch_input_ids,         ent_batch_shaking_tag,         rel_batch_shaking_tag = (batch_input_ids.to(device), 
                                 ent_batch_shaking_tag.to(device),
                                 rel_batch_shaking_tag.to(device)
                                )
    ...
    optimizer.zero_grad()
    if config["encoder"] == "BERT":
        pred_small_ent_shaking_outputs, ent_sampled_indices,         pred_small_rel_shaking_outputs, rel_sampled_indices = rel_extractor(batch_input_ids, 
    ...
    batch_small_ent_shaking_tag = ent_batch_shaking_tag.gather(1, ent_sampled_indices[:, :, None].repeat(1, 1, ent_tag_size))
    batch_small_rel_shaking_tag = rel_batch_shaking_tag.gather(1, rel_sampled_indices[:, :, None].repeat(1, 1, rel_tag_size))
    
    ent_loss = loss_func(pred_small_ent_shaking_outputs, batch_small_ent_shaking_tag)
    rel_loss = loss_func(pred_small_rel_shaking_outputs, batch_small_rel_shaking_tag)
...
#     print("bp: {}".format(time.time() - t1))
    pred_small_ent_shaking_tag = (pred_small_ent_shaking_outputs > 0.).long()
    ent_sample_acc = metrics.get_sample_accuracy(pred_small_ent_shaking_tag,
                                             batch_small_ent_shaking_tag)
    pred_small_rel_shaking_tag = (pred_small_rel_shaking_outputs > 0.).long()
    rel_sample_acc = metrics.get_sample_accuracy(pred_small_rel_shaking_tag,
                                             batch_small_rel_shaking_tag)
    
    return loss.item(), ent_sample_acc.item(), rel_sample_acc.item()

# valid step
def valid_step(batch_valid_data):
    if config["encoder"] == "BERT":
        sample_list, batch_input_ids,         batch_attention_mask, batch_token_type_ids,         tok2char_span_list, ent_batch_shaking_tag, rel_batch_shaking_tag = batch_valid_data
        ...
        sample_list, batch_input_ids,         tok2char_span_list, ent_batch_shaking_tag, rel_batch_shaking_tag = batch_valid_data
    ...
    with torch.no_grad():
        if config["encoder"] == "BERT":
            pred_ent_shaking_outputs, _,             pred_rel_shaking_outputs, _ = rel_extractor(batch_input_ids, 
                                                          batch_attention_mask, 
                                                          batch_token_type_ids, 
                                                         )
        elif config["encoder"] in {"BiLSTM", }:
    ...
    pred_ent_shaking_tag = (pred_ent_shaking_outputs > 0.).long()
    pred_rel_shaking_tag = (pred_rel_shaking_outputs > 0.).long()
    ent_sample_acc = metrics.get_sample_accuracy(pred_ent_shaking_tag,
                                                 ent_batch_shaking_tag)
    rel_sample_acc = metrics.get_sample_accuracy(pred_rel_shaking_tag,
                                                 rel_batch_shaking_tag)
    ...
    return ent_sample_acc.item(), rel_sample_acc.item(), cpg_dict
...
def train_n_valid(train_dataloader, dev_dataloader, optimizer, scheduler, num_epoch):  
    def train(dataloader, ep):
        ...
        t_ep = time.time()
        total_loss, total_ent_sample_acc, total_rel_sample_acc = 0., 0., 0.
        for batch_ind, batch_train_data in enumerate(dataloader):
            t_batch = time.time()
            
            loss, ent_sample_acc, rel_sample_acc = train_step(batch_train_data, optimizer)
            
            total_loss += loss
            total_ent_sample_acc += ent_sample_acc
            total_rel_sample_acc += rel_sample_acc
            ...
            avg_rel_sample_acc = total_rel_sample_acc / (batch_ind + 1)
            
            batch_print_format = "\rproject: {}, run_name: {}, Epoch: {}/{}, batch: {}/{}, train_loss: {}, " +                                 "ent_sample_acc: {}, rel_sample_acc: {}," +                                  "lr: {}, batch_time: {}, total_time: {} -------------"
            ...
            if config["logger"] == "wandb" and batch_ind % hyper_parameters["log_interval"] == 0:
                logger.log({
                    "train_loss": avg_loss,
                    "train_ent_shaking_seq_acc": avg_ent_sample_acc,
                    "train_rel_shaking_seq_acc": avg_rel_sample_acc,
    ...
    def valid(dataloader, ep):
        ...
        t_ep = time.time()
        total_ent_sample_acc, total_rel_sample_acc = 0., 0.
...
#         total_ent_correct_num, total_ent_pred_num, total_ent_gold_num = 0, 0, 0
        ...
        for batch_ind, batch_valid_data in enumerate(tqdm(dataloader, desc = "Validating")):
            ent_sample_acc, rel_sample_acc, cpg_dict = valid_step(batch_valid_data)
            total_ent_sample_acc += ent_sample_acc
            total_rel_sample_acc += rel_sample_acc
        ...
        avg_rel_sample_acc = total_rel_sample_acc / len(dataloader)
        ...
        if "rel_cpg" in total_cpg_dict:
            ...
            log_dict = {
                            "val_ent_shaking_tag_acc": avg_ent_sample_acc,
                            "val_rel_shaking_tag_acc": avg_rel_sample_acc,
