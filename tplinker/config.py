...
#     "encoder": "BiLSTM",
    "encoder": "BERT", 
    "hyper_parameters": {
        "shaking_type": "cat", # cat, cat_lstm, cln, cln_lstm; Experiments show that cat/cat_lstm work better with BiLSTM, while cln/cln_lstm work better with BERT. The results in the paper are produced by "cat". So, if you want to reproduce the results, "cat" is enough, no matter for BERT or BiLSTM.
...
#     "logger": "wandb", # if wandb, comment the following four lines
    ...
    "hyper_parameters": {
        ...
        "max_seq_len": 100,
        "sliding_len": 20,
        "loss_weight_recover_steps": 10000, # 6000 for webnlg, 10000 for nyt;to speed up the training process, the loss of EH-to-ET sequence is set higher than other sequences at the beginning, but it will recover in <loss_weight_recover_steps> steps.
        "scheduler": "CAWR", # Step
    },
}
